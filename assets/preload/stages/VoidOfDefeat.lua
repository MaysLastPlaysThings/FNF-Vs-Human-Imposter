...
} --LARGE SET OF PAD CONFIG, DO NOT TOUCH THIS
local groupHUD = {'Quote', 'Illus1', 'Illus2', 'Illus3', 'Illus4', 'Illus5', 'Illus6', 'TypeLine', 'WhiteLine', 'ComboString'} --OBJECTS TO TWEEN FOR MOBILE SPACE
local canTouch = false --CANNOT TAP PAD IF FALSE
local padGroup = {'numpad', 'numtext', 'numIm'} --3 LAYERS OF PADS
local scaleGuys = {
...
local oneTime = true --LIMITER, NOT USED IG
-------------------------------------------------------------
...
function mobileControls(type)
	if type == 'tweenHUD' then
		runTimer('delayMobileHUD', 2)
	end
...
function screenResOverlaps(target)
	local x, y = getMousePosition() -- Assuming there is a function to get the current mouse position
	local x2, y2 = getProperty(target .. '.x'), getProperty(target .. '.y')
	local width, height = getProperty(target .. '.width'), getProperty(target .. '.height')
...
-------------------------------------------------------------
	...
	for i = 1, 12 do
		...
		scaleObject('numpad' .. i, scaleGuys.x[i], scaleGuys.y[i])
	end
	for b = 1, 10 do
	...
	for e = 1, 10 do
		setProperty('numtext' .. e .. '.alpha', 0)
	end
	for f = 1, 2 do
		setProperty('numIm' .. f .. '.alpha', 0)
	end
	runHaxeCode([[
...
local shortener = {'numpad1', 'numpad2', 'numpad3', 'numpad4', 'numpad5', 'numpad6', 'numpad7', 'numpad8', 'numpad9', 'numpad10'}

function onUpdate(elapsed)
	--[MIXED CODE WITH MOBILE]
...
-------------------------------------------------------------
	...
	--[STANDALONE BIND INPUT](HARD WAY, BECAUSE THR SHORT METHOD CRASHES ON MOST DEVICES)
		for i = 1, #shortener do
			if screenResOverlaps(shortener[i]) and mouseClicked('left') and canTouch and #MaxCombo < 3 then
		...
		end]] --REMOVED BECAUSE TOO MUCH ITERATIONS, MAKES THE GAME CRASH
...
function onTimerCompleted(tag, loops, loopsLeft)
	--[MOBILE STUFF]
	--[[if tag == 'tweenHUD' and oneTime == true then
		oneTime = false
		mobileControls('tweenHUD')
		debugPrint('TWEENY COMPLETED')
	end]] --[NOT NECESSARY]
...
function onTweenCompleted(tag)
	--[MOBILE STUFF]
	if tag == 'tweenD1' or tag == 'tweenG1' then
		canTouch = true
		runHaxeCode([[
